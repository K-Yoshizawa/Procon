    ...
    while(Q--){
        ...
        if(t == 0){
            ...
            for(auto &path : hld.PathQuery(u, v)){
                if(path.reverse) ans = Affine::left_merge(ans, seg2.Fold(path.head_index, path.tail_index));
                else ans = Affine::left_merge(ans, seg1.Fold(path.head_index, path.tail_index));
            }
            cout << ans.a * x + ans.b << '\n';
        }
    }
